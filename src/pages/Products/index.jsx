...
const Products = ({ products, dispatch, match, history, location, loading }) => {
  ...
  const columns = [
...

    {
      ...
      render: (_, record) => (
        <p>
          {record?.availableToPromiseTotal === 0
            ? 'Out of stock'
            : record?.availableToPromiseTotal
            ? `${record?.availableToPromiseTotal} in stock ${
                record?.variantCount ? `for ${record?.variantCount} variants` : ''
              }`
